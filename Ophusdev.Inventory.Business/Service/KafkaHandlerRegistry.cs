...
namespace Ophusdev.Inventory.Business.Services
{
    public class KafkaHandlerRegistry : IKafkaHandlerRegistry
    {
        ...
        public void RegisterTypedHandler<TMessage>(string topicKey, Func<IInventoryService, TMessage, Task> handler)
        {
            string topicName = _topicTranslator.GetTopicName(topicKey);

            IKafkaHandlerRegistry.RawKafkaMessageHandler rawHandler = async (inventoryService, topic, message) =>
            {
                try
                {
                    ...
                    {
                        await handler(inventoryService, typedMessage);
                    }
                    else
                    {
